...
def draw_result(y_val, y_test, val_results, test_results, key, current_dataset):
    ...
    roc_data_path = f"../../data/ROC/LR/{key}"
    ...
    plt.tight_layout()
    roc_pic_path = f"../../images/LR/ROC_AUC/Dataset{current_dataset}"
    ...
    if os.path.exists(f"{json_file}/LR_data.json"):
        with open((f"{json_file}/LR_data.json"), "r", encoding="utf-8") as f:
            try:
    ...
    # 写回 JSON 文件
    with open((f"{json_file}/LR_data.json"), "w", encoding="utf-8") as f:
...
def draw_shap(final_model, X_train, X_test, current_dataset, key):
    ...
    plt.tight_layout()
    shap_path1 = f"../../images/LR/shap/Dataset{current_dataset}"

...
def draw_result(y_val, y_test, val_results, test_results, key, current_dataset):
    ...
    roc_data_path = f"../../data/ROC/LR+XGBoost3/{key}"
    ...
    plt.tight_layout()
    roc_pic_path = f"../../images/LR_XGBoost3/ROC_AUC/Dataset{current_dataset}"
    ...
    if os.path.exists(f"{json_file}/LR_XGBoost3_data.json"):
        with open(f"{json_file}/LR_XGBoost3_data.json", "r", encoding="utf-8") as f:
            try:
                existing_data = json.load(f)
                if f'LR_XGBoost3_Dataset{current_dataset}' not in existing_data:
                    existing_data[f'LR_XGBoost3_Dataset{current_dataset}'] = {}
    ...
    # 追加新的数据
    existing_data[f'LR_XGBoost3_Dataset{current_dataset}'][model] = data_to_append

    # 写回 JSON 文件
    with open(f"{json_file}/LR_XGBoost3_data.json", "w", encoding="utf-8") as f:

...
def draw_result(y_val, y_test, val_results, test_results, key, current_dataset):
    ...
    roc_data_path = f"../../data/ROC/LR+XGBoost1/{key}"
    ...
    plt.tight_layout()
    roc_pic_path = f"../../images/LR_XGBoost1/ROC_AUC/Dataset{current_dataset}"
    ...
    if os.path.exists(f"{json_file}/LR_XGBoost1_data.json"):
        with open(f"{json_file}/LR_XGBoost1_data.json", "r", encoding="utf-8") as f:
            try:
    ...
    # 写回 JSON 文件
    with open(f"{json_file}/LR_XGBoost1_data.json", "w", encoding="utf-8") as f:

...
def create_pipeline(X_train, y_train):
    ...
    model_pipeline = Pipeline(steps=[
        ...
        ('classifier', SVC(probability=True, random_state=42, max_iter=1000,kernel='rbf'))
    ])
...
def draw_result(y_val, y_test, val_results, test_results, key, current_dataset):
    ...
    roc_data_path = f"../../data/ROC/SVM/{key}"
    ...
    plt.title(f'{key} - SVM ROC曲线', fontsize=14, fontweight='bold')
    plt.legend(loc="lower right", fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    roc_pic_path = f"../../images/SVM/ROC_AUC/Dataset{current_dataset}"
    ...
    if os.path.exists(f"{json_file}/SVM_data.json"):
        with open(f"{json_file}/SVM_data.json", "r", encoding="utf-8") as f:
            try:
                existing_data = json.load(f)
                if f'SVM_Dataset{current_dataset}' not in existing_data:
                    existing_data[f'SVM_Dataset{current_dataset}'] = {}
    ...
    # 追加新的数据
    existing_data[f'SVM_Dataset{current_dataset}'][model] = data_to_append

    # 写回 JSON 文件
    with open(f"{json_file}/SVM_data.json", "w", encoding="utf-8") as f:
...
def draw_shap(final_model, X_train, X_test, current_dataset, key):
    ...
    if len(X_test) < 50:
        ...
        X_actual_test = X_test.sample(50, random_state=42)
    # 计算 SHAP 值（对测试集）
    shap_values = explainer.shap_values(X_actual_test, nsamples=100)
    ...
    plt.tight_layout()
    shap.summary_plot(shap_values, X_actual_test, plot_type="bar", show=False)
    shap_path1 = f"../../images/SVM/shap/Dataset{current_dataset}"
    ...
    plt.tight_layout()
    print(f"\n===== SHAP 特征分布 (蜂群图): =====")
    shap.summary_plot(shap_values, X_actual_test, show=False)
